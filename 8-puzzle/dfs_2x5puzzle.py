...
def dfs_puzzle(start_state, goal_state):
    """
    깊이 우선 탐색(DFS)을 이용해 퍼즐을 푸는 함수.
    - stack을 이용한 반복 DFS
    - (상태, 경로)를 튜플로 저장
    - 방문 집합(visited)을 이용해 중복 상태 방지
    """
    stack = [(start_state, [])]
    ...
    while stack:
        ...
        print(f"[DFS - Expand #{expand_count}] (깊이={len(path)})")
